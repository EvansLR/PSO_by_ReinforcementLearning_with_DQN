...
if __name__=="__main__":
    ...
    for i_episode in range(10):
        ...
        while True:
            ...
            if done:
                ...
                reward_sum = sum(RL._discount_and_norm_rewards())
                print("episode:", i_episode, " , reward", reward_sum, " , best_fitness", env.gbestFit)


                if env.gbestFit < RL_best_fit and env.gbestPunish<0.0001:
    ...
    for i in range(10):
        ...
        if s_pso.gbestFit<standard_best and s_pso.gbestPunish<0.0001:
            s_pso_best_pos=s_pso.gbest
            standard_best=s_pso.gbestFit
            s_curve=s_pso.fitness_list
    print('标准粒子群最优位置和最优解',s_pso_best_pos)
    print(standard_best)
    ...
    plt.ylabel('Target_Func_Value')

    plt.title('f(x)=G01', size=20)
    plt.legend()
    plt.show()